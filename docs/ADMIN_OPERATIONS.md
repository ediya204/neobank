...
> 客户法币转出、USDT 充值和全部客户转出仍关闭。客户 OTC 使用服务端五秒
> 短效报价：首次请求不占用余额，客户确认后由 PostgreSQL Core 原子记账并
> 直接完成，不进入 Admin 审批。Admin 的 OTC 页面只读核对成交记录；Partner
> 机器 API 的手动 OTC 写入不在本次开放范围内。
...
| 发起 USDT 转出           | USDT 出款  | 审核网络和地址；显式提交 Cregis；由签名回调决定完成或失败                               | 提交时占用指定网络 USDT；完成后仅扣该网络；拒绝或失败进入自动释放队列，达到 `released` 后才显示最终状态 |
| 发起 USD → USDT OTC      | 自动兑换   | 服务端生成五秒报价；客户确认后即时完成，管理员只读核对                                  | 扣 USD，按所选网络增加已包含当前报价费率的 USDT                   |
| 发起 USDT → USD OTC      | 自动兑换   | 服务端生成五秒报价；客户确认后即时完成，管理员只读核对                                  | 仅扣所选网络 USDT，增加已包含当前报价费率的 USD                   |
...
管理员可在客户上下文中修正开户基础资料和已录入的 VA 账户资料。旧 VA/Partner
界面的 Sumsub 链接人工录入不属于 Neobank 个人开户流程；个人开户必须由服务端创建
Applicant 并通过签名 Webhook/主动同步取得结果，后台不得粘贴链接代替关联或核验。
修正只能通过业务 API 完成；每次修正必须写入审计日志，并保留 `application_id`、
操作类型和变更时间。资金余额不属于客户资料，不能在此处编辑。
...
- 新个人申请展示 Sumsub Level、环境、Applicant ID、最近同步时间，以及护照、活体人脸、
  住址证明三项状态；`moderationComment` 可供客户沟通，`clientComment` 仅后台可见；
- 新个人申请只有三项及申请级结果全部 GREEN 后才允许提交人工批准；Sumsub GREEN 本身
  不得批准 KYC 或开通账户。接入前的个人申请和企业 KYB 保持原人工路径；
...
只要卖出或买入一侧是 USDT，就必须指定网络。首次请求由服务端取得 FastForex
行情并生成五秒有效的 `DRAFT` 报价，不冻结或改变余额。客户必须在倒计时内显式
确认；确认请求在同一个 PostgreSQL 串行化事务中重新校验客户归属和余额，完成卖出
扣账、买入净额入账及平衡分录，并直接进入 `COMPLETED`。超时报价进入
`CANCELLED`，不能成交且不产生账本。Admin 页面只用于对账和审计，不提供审批、
处理或修改入口；直接创建 OTC Operation 的旁路被拒绝。
...
```text
DRAFT（五秒报价，不占用余额） → COMPLETED（客户确认后原子记账）
                         └──────→ CANCELLED（报价过期）
```

确认请求才是实际提交点；成交记录、平衡账本分录和审计快照原子写入，`checkerId`
与 `approvedAt` 保持为空。重复确认同一已完成报价返回同一结果，不能重复记账。
...
8. Ethan 分别为 USD → TRON USDT 和 TRON USDT → USD 获取五秒 OTC 报价；未确认时余额不变，倒计时内点击确认后无需审批并按指定网络原子扣账和入账，过期确认被拒绝。
...
15. OTC 首次请求只返回带到期时间的 `DRAFT` 且不占用余额；五秒内确认返回
    `COMPLETED` 后，在订单详情、统一交易记录和账本核对卖出、费率与净买入字段；
    `checkerId`/`approvedAt` 为空，Admin 页面不得出现审批或处理按钮。
