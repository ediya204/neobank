...
客户详情必须根据已配置的 `VIRTUAL_ACCOUNT`、`POBO_PAYOUT` 和
`PLATFORM_PAYOUT` 通道及其支持币种生成完整的法币转出规则清单，不能只展示数据库中
已经启用的默认规则。每一行都显示机构默认、客户实际生效值、规则版本和通道状态；管理员
可设置客户专属固定费用，也可停用专属规则并回退机构默认。机构默认缺失或已停用时，页面
明确显示“暂不可转出”并提供机构默认管理入口，不得静默补成零手续费。客户专属规则可以在
机构默认缺失时单独启用，但停用该专属规则后，该路径会因没有生效规则而阻止新转出。
