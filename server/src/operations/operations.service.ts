...
export class OperationsService {
  ...
  approvals(organizationId: string, userId: string) {
    return this.db.operation.findMany({
      where: {
  ...
  async get(id: string) {
    const operation = await this.db.operation.findUnique({
      where: { id },
      include: { ...operationInclude, journals: { include: { lines: { include: { account: true } } } } },
    });
  ...
  async create(input: CreateOperationInput, makerId: string) {
    ...
    const [maker, customer, source, target, channel, beneficiary] = await Promise.all([
      ...
      input.sourceAccountId ? this.db.account.findUnique({ where: { id: input.sourceAccountId } }) : null,
      input.targetAccountId ? this.db.account.findUnique({ where: { id: input.targetAccountId } }) : null,
      input.channelId ? this.db.fundingChannel.findUnique({ where: { id: input.channelId } }) : null,
      input.beneficiaryId ? this.db.beneficiary.findUnique({ where: { id: input.beneficiaryId } }) : null,
    ]);
    ...
    if (channel && channel.organizationId !== customer.organizationId) throw new ForbiddenException('cross_tenant_channel');
    this.validateShape(input, source, target, channel, beneficiary);

    const reference = `OP-${new Date().toISOString().slice(0, 10).replaceAll('-', '')}-${randomUUID().slice(0, 8).toUpperCase()}`;
    const frozen = input.type === 'PAYOUT' || input.type === 'INTERNAL_TRANSFER' || input.type === 'FX' || input.type === 'OTC' ||
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (frozen && source) await this.freeze(tx, source.id, reserve);
  ...
  async approve(id: string, checkerId: string) {
    return this.db.$transaction(async (tx) => {
      const operation = await tx.operation.findUnique({ where: { id }, include: operationInclude });
      if (!operation) throw new NotFoundException('operation_not_found');
      if (operation.status !== 'SUBMITTED') throw new ConflictException('operation_not_pending_approval');
      if (operation.makerId === checkerId) throw new ForbiddenException('maker_cannot_approve_own_operation');
  ...
  async reject(id: string, reason: string, checkerId: string) {
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (this.usesReservation(operation) && operation.sourceAccountId) {
        await this.unfreeze(tx, operation.sourceAccountId, operation.amount.add(operation.feeAmount));
      }
      return tx.operation.update({
        where: { id },
        data: { status: 'REJECTED', checkerId, approvedAt: new Date(), rejectionReason: reason.trim() },
        include: operationInclude,
      });
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
  }
  ...
  private validateShape(
    ...
    source: { id: string; customerId: string | null; kind: AccountKind; status: AccountStatus; currency: Currency } | null,
    target: { id: string; customerId: string | null; kind: AccountKind; status: AccountStatus; currency: Currency } | null,
    ...
    if (target && target.status !== 'ACTIVE') throw new BadRequestException('invalid_target_account');
    ...
    if (input.type === 'DEPOSIT') {
      ...
      if (target.customerId !== input.customerId) throw new BadRequestException('deposit_target_customer_mismatch');
    }
    if (input.type === 'PAYOUT') {
      ...
      const expectedChannel: ChannelType = input.payoutMethod === 'VA' ? 'VA_PAYOUT' : input.payoutMethod === 'POBO' ? 'POBO_PAYOUT' : 'PLATFORM_PAYOUT';
      if (source.kind !== expectedKind || channel.type !== expectedChannel) {
        throw new BadRequestException('payout_source_or_channel_mismatch');
      }
      if (source.customerId !== input.customerId || !beneficiary || !beneficiary.active ||
          beneficiary.customerId !== input.customerId || beneficiary.currency !== input.currency) {
        throw new BadRequestException('payout_customer_or_beneficiary_mismatch');
      }
    }
    if (input.type === 'INTERNAL_TRANSFER') {
      if (!source || !target || !target.customerId || source.id === target.id || target.currency !== input.currency) {
        throw new BadRequestException('invalid_internal_transfer_accounts');
      }
    }
    if (input.type === 'FX' || input.type === 'OTC') {
      if (!source || !target || !input.quoteCurrency || target.currency !== input.quoteCurrency || input.currency === input.quoteCurrency) {
    ...
    if (input.type === 'ADJUSTMENT') {
      if (!input.adjustmentDirection || (input.adjustmentDirection === 'CREDIT' ? !target : !source)) {
  ...
  private async postApprovedOperation(tx: Prisma.TransactionClient, operation: Operation) {
    if (operation.type === 'DEPOSIT' || (operation.type === 'ADJUSTMENT' && operation.adjustmentDirection === 'CREDIT')) {
    ...
    if (operation.type === 'INTERNAL_TRANSFER') {
      if (!operation.sourceAccountId || !operation.targetAccountId) throw new BadRequestException('transfer_accounts_required');
      await this.consumeFrozen(tx, operation.sourceAccountId, operation.amount.add(operation.feeAmount));
      await this.creditAvailable(tx, operation.targetAccountId, operation.amount);
      await this.journal(tx, operation, [
  ...
  private async postConversion(tx: Prisma.TransactionClient, operation: Operation) {
    ...
    const quoteAmount = operation.amount.mul(rate.sellRate).mul(new Prisma.Decimal(1).sub(new Prisma.Decimal(rate.feeBps).div(10000)));
    ...
    await tx.operation.update({ where: { id: operation.id }, data: { quoteAmount, rate: rate.sellRate, rateVersionId: rate.id } });
    await this.postFee(tx, operation);
  }
  ...
  private async postFee(tx: Prisma.TransactionClient, operation: Operation) {
    ...
    const fee = await tx.account.findFirst({ where: { kind: 'FEE_REVENUE', currency: operation.currency } });
  ...
  private async journal(
    ...
    if ([...totals.values()].some((total) => !total.isZero())) throw new ConflictException('unbalanced_journal');
    await tx.journalEntry.create({
      data: {
        reference: `${operation.reference}-${suffix}`,
        operationId: operation.id,
        description: operation.narrative || `${operation.type} ${operation.reference}`,
        lines: { create: lines.map(([accountId, side, currency, amount]) => ({ accountId, side, currency, amount })) },
      },
    });
  }

  private clearingAccount(tx: Prisma.TransactionClient, currency: Currency) {
    return tx.account.findFirstOrThrow({ where: { kind: 'PLATFORM_CLEARING', currency, status: 'ACTIVE' } });
  }
  ...
  private async freeze(tx: Prisma.TransactionClient, accountId: string, amount: Prisma.Decimal) {
    const result = await tx.account.updateMany({
      ...
      data: { availableBalance: { decrement: amount }, frozenBalance: { increment: amount }, version: { increment: 1 } },
    });
  ...
  private async unfreeze(tx: Prisma.TransactionClient, accountId: string, amount: Prisma.Decimal) {
    const result = await tx.account.updateMany({
      ...
      data: { availableBalance: { increment: amount }, frozenBalance: { decrement: amount }, version: { increment: 1 } },
    });
    if (result.count !== 1) throw new ConflictException('invalid_frozen_balance');
  }

  private async consumeFrozen(tx: Prisma.TransactionClient, accountId: string, amount: Prisma.Decimal) {
    const result = await tx.account.updateMany({
      where: { id: accountId, frozenBalance: { gte: amount } },
  ...
  private async creditAvailable(tx: Prisma.TransactionClient, accountId: string, amount: Prisma.Decimal) {
    const result = await tx.account.updateMany({
  ...
  private usesReservation(operation: Operation) {
    return operation.type === 'PAYOUT' || operation.type === 'INTERNAL_TRANSFER' || operation.type === 'FX' || operation.type === 'OTC' ||
  ...
  private async requireRole(tx: Prisma.TransactionClient, userId: string, roles: UserRole[]) {
    ...
    if (!user || !user.active || !roles.includes(user.role)) throw new ForbiddenException('insufficient_role');
    return user;
  }
  ...
  private positiveMoney(value: string, field: string, currency: Currency) {
    ...
    if (amount.decimalPlaces() > precision) throw new BadRequestException(`${field}_precision_exceeded`);
    return amount;
  }
}
