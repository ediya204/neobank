...
var tables = []tableSpec{
	...
	{Name: "customer_applications", PrimaryKey: "id", Columns: fields(`
...
func RunFromEnvironment(ctx context.Context) (Manifest, error) {
	sourceURL := strings.TrimSpace(os.Getenv("SOURCE_D1_GATEWAY_URL"))
	sourceSecret := strings.TrimSpace(os.Getenv("SOURCE_D1_GATEWAY_SECRET"))
	if sourceURL == "" {
		sourceURL = strings.TrimSpace(os.Getenv("D1_GATEWAY_URL"))
	}
	if sourceSecret == "" {
		sourceSecret = strings.TrimSpace(os.Getenv("D1_GATEWAY_SECRET"))
	}
	source, err := d1.New(sourceURL, sourceSecret)
	if err != nil {
		return Manifest{}, err
	}
...
func Run(ctx context.Context, source Database, target Database) (Manifest, error) {
	for name, query := range map[string]string{
		...
		if preflightErr != nil || len(rows) != 1 || integer(rows[0]["count"]) != 0 {
			return Manifest{}, fmt.Errorf("D1 source preflight %s failed", name)
		}
	}
	...
	for _, table := range tables {
		...
		if countErr != nil || len(countRows) != 1 {
			return Manifest{}, fmt.Errorf("count target table %s", table.Name)
		}
		if integer(countRows[0]["count"]) != 0 {
		...
		if queryErr != nil {
			return Manifest{}, fmt.Errorf("read D1 source table %s: %w", table.Name, queryErr)
		}
		...
		for _, row := range rows {
			...
			for _, column := range table.Columns {
				...
				if valueErr != nil {
					return Manifest{}, fmt.Errorf("normalize %s.%s: %w", table.Name, column, valueErr)
				}
	...
	if !targetPopulated && len(statements) != 0 {
		if _, err := target.Batch(ctx, statements...); err != nil {
			return Manifest{}, fmt.Errorf("copy D1 snapshot into postgres: %w", err)
		}
	}

	result := Manifest{
		GeneratedAt: time.Now().UTC().Format(time.RFC3339Nano),
		Source:      "d1-maintenance-snapshot",
		Target:      "render-postgres",
		Copied:      !targetPopulated && len(statements) != 0,
	}
	for _, table := range tables {
		query := fmt.Sprintf("SELECT %s FROM %s ORDER BY %s", selectColumns(table), table.Name, table.PrimaryKey)
		targetRows, queryErr := target.Query(ctx, query)
		if queryErr != nil {
			return Manifest{}, fmt.Errorf("verify postgres table %s: %w", table.Name, queryErr)
		}
		...
		if len(sourceRows[table.Name]) != len(targetRows) || sourceHash != targetHash {
			return Manifest{}, fmt.Errorf("verification mismatch for %s", table.Name)
		}
