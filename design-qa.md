...
范围：用户指定的开户表单布局与按钮尺寸调整；本地组件预览，不代表生产业务验收。

- 对照：用户提供的原始开户弹窗截图与从当前源码提取的实际 CustomerDialog；同次图像输入中比较。参考图缩放比例未知，按表单区域及相对布局比较，不作像素级一致性声明。
- 预览： http://127.0.0.1:4173/ ，使用项目现有 MUI 字体、配色、间距及组件覆盖配置；无生产接口连接。
- 桌面 866×731：姓名及密码双列，客户资料／联系方式／登录安全分组；电话区号 112px，不再被压缩；内容区域可滚动，底部操作栏保留可见。
- 窄屏 390×844：企业字段单列，邮箱独占一行，区号和电话同组；滚动后密码、提示与提交按钮均可达，无可见横向溢出。
- 交互：个人／企业切换显示正确字段；英文姓名 O'Neil 与 Jean-Luc 通过输入校验；取消与重新打开可用。控制台无 error。
- 按钮：页面“发起开户”源码限定 36px 高并居中，避免被标题区纵向拉伸；提交按钮 36px 高。页面完整鉴权流程不在组件预览范围内。
- 保留原有颜色与组件风格，无新增图片或图标资源；文案增加明确的“客户登录邮箱”。
- 尚未发布；预览提交仅显示本地提示，不创建客户。未执行生产开户。

---

# 后台开户国家地区选择 — 2026-09-14

final result: passed

- 范围：常住／企业注册国家地区、个人国籍，复用客户端注册的五项国家代码及翻译。
- 在同次图像输入中对比用户原截图与 866×731 本地实际组件预览；按表单区域比较，原截图缩放未知，不作像素一致性声明。
- 原有双列、字体、颜色、间距和按钮保持；SG 显示为新加坡，“国籍代码”改为“国籍”，无新增图片资源。
- 浏览器验证国籍菜单包含中国香港、新加坡、中国大陆、英国、美国，选择中国大陆后字段正确更新。
- 本地预览无生产接口连接，未提交生产开户，尚未部署。
